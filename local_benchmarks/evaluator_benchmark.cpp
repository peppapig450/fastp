...
void applyAdapterContamination(std::string&     sequence,
    ...
    if (sequence.size() > minInsert + minAdapter) {
        ...
        sequence = sequence.substr(0, cut) + std::string(adapter.substr(0, adapterToTake));
        quality  = quality.substr(0, cut) + std::string(adapterToTake, 'I');
    }
}
...
public:
    ...
    auto generateDataset(const std::string& name,
                         ...
                         const std::string& refFasta          = "") -> std::string {
...
private:
    ...
    auto generateDatasetImpl(const std::string& name,
                             ...
                             bool               withVariants,
                             const std::string& refFasta) -> DatasetReturn<IsPairedEnd>;
    ...
    auto validateReferenceGenome(const std::string& refFasta) -> std::string {
        if (!fs::exists(refFasta)) {
            throw std::runtime_error("Reference FASTA not found: " + refFasta);
        }

        // very light sanity check: first char '>
        std::ifstream in(refFasta);
        char          firstChar = 0;
...
auto MasonDataGenerator::generateDatasetImpl(const std::string& name,
    ...
    const auto baseRef =
        (refFasta.empty() ? generateReferenceGenome(name) : validateReferenceGenome(refFasta));
    ...
    const auto simRef = (withVariants && !vcfPath.empty() && mason_.materialize)
                            ? materializeGenome(baseRef, vcfPath, name)
                            : baseRef;
