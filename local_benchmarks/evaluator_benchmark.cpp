...
static void BM_CheckKnownAdapters_Legacy_WithFiltering(benchmark::State& state) {
    ...
    auto legacyWithFiltering = [&evaluator](const std::vector<std::unique_ptr<Read>>& reads) -> std::string {
        const auto& knownAdapters = adapters::getKnown();
        std::unordered_map<std::string, int> stats;
        ...
        for (const auto& read : reads) {
            ...
            if (seq.length() >= kStartPos + static_cast<std::size_t>(kKeyLen)) {
                ...
                for (std::size_t pos = kStartPos; pos <= limit && !shouldSkipRead; ++pos) {
                    rolling = EvaluatorAccess::seq2int(evaluator, seq, static_cast<int>(pos), kKeyLen, rolling);
                    // Note: We can't access isLowComplexityKey directly, so we'll skip this optimization
                    // This gives the legacy method a slight advantage in this benchmark
                }
            }
...
static void BM_AdapterMatching_Legacy_Direct(benchmark::State& state) {
    ...
    auto fastq = benchmark_util::makeAdapterPrefixFastq("legacy_direct", readCount, readLength, 1.0);
    auto reads = benchmark_util::loadReads(fastq, readCount);
...
static void BM_CheckKnownAdapters_Legacy_ParamContam(benchmark::State& state) {
    ...
    auto& generator = benchmark_data::getGenerator();
    const std::string tag = std::format("legacy_contam_rr{}_len{}_c{}", readCount, readLength, contam_perm);
